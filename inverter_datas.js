...
workbook.xlsx.readFile(SOURCE_FILE)
    .then(function () {
        ...
        while (i <= worksheet.rowCount) {
            var cell_closed_at = worksheet.getCell(STORE_CLOSED_AT + i).value
            var cell_created_at = worksheet.getCell(CREATED_AT + i).value
            if (cell_closed_at != null) {
                var partes = cell_closed_at.split(" ");
                // var date = new Date(partes[0])
                var partes_partes = partes[0].split("/")
                var dataAlterada = partes_partes[1] + "/" + partes_partes[0] + "/" + partes_partes[2]
                worksheet.getCell(STORE_CLOSED_AT + i).value = dataAlterada + " " + partes[1]
            }

            if (cell_created_at != null) {
                var partes = cell_created_at.split(" ");
                // var date = new Date(partes[0])
                var partes_partes = partes[0].split("/")
                var dataAlterada = partes_partes[1] + "/" + partes_partes[0] + "/" + partes_partes[2]
                worksheet.getCell(CREATED_AT + i).value = dataAlterada + " " + partes[1]
            }
